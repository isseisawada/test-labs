...
- date: YYYY-MM-DD。**実際に利用・購入した日**を最優先で読む。
  * 領収書の「発行日」「印刷日」「ダウンロード日」と「利用日」が違う場合は必ず**利用日**を採る
  * サブスク・クラウド利用料など期間に対する請求は、その**サービス提供期間の月**の日付にする
    （例: 「2026年8月分」の請求書が9月発行なら {year}-{month:02d}-31 のように8月の日付にする）
  * 年が読み取れない/不自然なら {year} 年とする
- vendor: 店名・発行元（正式名称）
